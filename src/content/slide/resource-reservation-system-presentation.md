...
Spring Boot Server
  ├─ auth: 회원가입, 로그인, JWT
  ├─ resource: 자원 종류, 자원, 시간대
  ├─ reservation: 예약, 취소, 정책, 이벤트
  ├─ waitlist: 대기열과 자동 승격
  ├─ statistics: 예약 통계
  └─ stress: 외부 부하 시나리오 모니터링
        │ JPA
        ▼
H2 File Database
```
...
프런트엔드는 사용자 화면, 관리자 화면, 데모 화면으로 나뉘고, 모든 데이터는 REST API를 통해 서버와 주고받습니다.
서버는 인증, 자원, 예약, 대기열, 통계, stress 모니터링 영역으로 분리되어 있습니다.
-->
...
application          Service, Policy, Event Publisher
adapter.out          Repository

domain               Entity, Enum, 상태 변경 메서드
```

- Controller는 HTTP 요청과 응답 담당
- Service는 비즈니스 흐름과 트랜잭션 담당
- Repository는 DB 접근 담당
- Domain은 예약, 자원, 대기열 상태 표현

<!--
발표 메모:
백엔드는 계층을 분리했습니다.
Controller는 REST API 입구 역할만 하고, 실제 비즈니스 규칙은 Service에 둡니다.
DB 접근은 Repository로 분리하고, Reservation이나 WaitlistEntry 같은 도메인 객체는 자신의 상태 변경을 메서드로 표현합니다.
-->
...
같은 슬롯에 중복 예약이 생기지 않도록 여러 방어선을 두었습니다.

- `@Transactional`로 예약 흐름을 하나의 작업 단위로 처리
- 예약 슬롯 조회 시 pessimistic lock 사용
- DB unique 제약으로 최종 무결성 보장
- `DataIntegrityViolationException`을 예약 충돌로 변환
- 대기열은 FIFO 순서로 자동 승격
...
이를 위해 트랜잭션과 pessimistic lock을 사용하고, 마지막 방어선으로 DB unique 제약을 둡니다.
또 충돌이 발생했을 때는 서버 오류가 아니라 예약 충돌 또는 대기열 등록 흐름으로 처리했습니다.
-->
...
1. `admin / admin1234`로 로그인
2. 100석 공연 생성
3. `user1 / password123`으로 좌석 예매
4. `user2 / password123`으로 같은 좌석 대기열 등록
5. `user1` 예약 취소
6. `user2`가 자동으로 예매 승격되는지 확인
7. 관리자 화면에서 이력과 통계 확인
